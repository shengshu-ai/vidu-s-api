...
补齐当前插件市场校验要求的源码仓库和维护者联系方式，包含下述 0.0.2 修复，保持现有接口兼容。

## 0.0.2 更新

清理凭证验证、API 请求及超时错误中的原始异常链，避免 Dify 日志记录敏感信息；包含非法字符的 API Key 会在发出 HTTP 请求前被拒绝。安装包排除了维护者发布说明及本地生成文件。现有 S1 工具、参数和 Workflow 输出保持兼容。

## 源码与支持

- 源码：https://github.com/shengshu-ai/vidu-s-api
- Bug 反馈与支持：https://github.com/shengshu-ai/vidu-s-api/issues
